...
public class LSYAlbum {
    ...
    public func setupAlbumGroups(albumGroups:albumGroupsBlock){
        var groups = NSMutableArray()
        var resultBlock:ALAssetsLibraryGroupsEnumerationResultsBlock = { (group:ALAssetsGroup!,stop:UnsafeMutablePointer<ObjCBool>)->Void in
            if group != nil {
                group.setAssetsFilter(self.assetsFilter)
                var groupType = group.valueForProperty(ALAssetsGroupPropertyType).integerValue
    ...
    public func setupAlbumAssets(group:ALAssetsGroup,albumAssets:albumAssetsBlock){
        var assets = NSMutableArray()
        group.setAssetsFilter(self.assetsFilter)
        var assetCount = group.numberOfAssets()
        var resultBlock :ALAssetsGroupEnumerationResultsBlock = { (asset:ALAsset!,index:Int,stop:UnsafeMutablePointer<ObjCBool>)->Void in
            if asset != nil {
                var model:LSYAlbumModel = LSYAlbumModel(data: asset)
                assets.addObject(model)
                var assetType:String! = model.asset.valueForProperty(ALAssetPropertyType) as? String
