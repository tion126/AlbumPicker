...
@objc class LSYAssetPreview: UIViewController {
    ...
    private func setAssets(){
        ...
        for var i = 0;i<self.assets.count;i++ {
            var previewItem :LSYAssetPreviewItem = LSYAssetPreviewItem(frame: CGRectMake(LSYSwiftDefine.ViewSize(self.view).width * CGFloat(i),0, LSYSwiftDefine.ViewSize(self.view).width, LSYSwiftDefine.ViewSize(self.view).height))
            previewItem.delegate = self
            var model:LSYAlbumModel = self.assets[i] as!LSYAlbumModel
